...
extension HMPopup {
    ...
    /// 弹出Alert 弹出View的高度由约束自动撑
    /// - Parameters:
    ///   - name: 弹出层的name 消失时需一致  默认nil表示消失正在显示的
    ///   - alert: 自定义alert 高度由约束自动撑
    ///   - vc: 从某个VC弹出
    ///   - size: 弹框内容布局尺寸
    ...
    /// - Returns: HMPopupMask
    @discardableResult
    public static func showAlert(_ name: String? = nil,
                                 _ alert: UIView,
                                 for vc: UIViewController? = nil,
                                 size: HMPopupSize = .layoutFit(leading: 30),
                                 dismissWhenTapMask: Bool = true,
                                 completion: (() -> Void)? = nil) -> HMPopupMaskVC {
        let attribute = HMPopupAnimationFactory.alert(name: name, popupView: alert, size: size)
        attribute.dismissWhenTapMask = dismissWhenTapMask
        let mask = self.show(attribute: attribute, for: vc, completion: completion)
        return mask
    }
    
    /// 从底部弹出actionSheet 弹出actionSheet的高度由约束自动撑
    /// - Parameters:
    ///   - name: 弹出层的name 消失时需一致  默认nil表示消失正在显示的
    ///   - actionSheet: 自定义actionSheet 高度由约束自动撑
    ///   - vc: 从某个VC弹出
    ///   - size: 弹框内容布局尺寸
    ///   - dismissWhenTapMask: 点击背景是否消失
    ///   - completion: 完成回调
    /// - Returns: HMPopupMask
    @discardableResult
    public static func showActionSheet( _ name: String? = nil,
                                        _ actionSheet: UIView,
                                        for vc: UIViewController? = nil,
                                        size: HMPopupSize = .layoutFit(leading: 0),
                                        dismissWhenTapMask: Bool = true,
                                        completion: (() -> Void)? = nil) -> HMPopupMaskVC {
        let attribute = HMPopupAnimationFactory.present(name: name, popupView: actionSheet, size: size)
        attribute.dismissWhenTapMask = dismissWhenTapMask
        let mask = self.show(attribute: attribute, for: vc, completion: completion)
        return mask
    }
