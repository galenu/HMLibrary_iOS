...
extension HMPopup {
    ...
    public class func getContainerVC(_ vc: UIViewController?, isPresented: Bool = true) -> UIViewController? {
        if isPresented {
            if let vc = vc {
    ...
    public class func allPresentedControllers(for vc: UIViewController) -> [UIViewController] {
        var preVC = vc
    ...
    public class func allPopupMask(name: String? = nil, for vc: UIViewController? = nil) -> [HMPopupMaskVC]? {
        ...
        guard let allPopups = self.allPresentedControllers(for: containerVC).filter({ vc in
            return vc.isKind(of: HMPopupMaskVC.self)
        }) as? [HMPopupMaskVC] else {
            return nil
        }
