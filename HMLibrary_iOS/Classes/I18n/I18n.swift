...
public class I18n {
    ...
    /// 各个模块的多语言.lproj文件的bundle集合 [i18nBundleId:  I18nBundle]
    public private(set) var lprojBundleMap = [String: I18nBundle]()
    ...
    public class func localized(_ key: String,
        ...
        // 如果没有国际化对应语言，设置默认语言
        var lprojBundle = bundle.lprojBundle ?? bundle.defaultBundleWhenValueEmpty
        guard let lprojBundle = lprojBundle else { return key }
        var localizedString = NSLocalizedString(key, tableName: tableName, bundle: lprojBundle, comment: comment)
        // 如果国际化语言没有值，再次读取英文默认值
        if let defaultBundle = bundle.defaultBundleWhenValueEmpty, localizedString == key {
            localizedString = NSLocalizedString(key, tableName: tableName, bundle: defaultBundle, comment: comment)
        }
    ...
    public static func updateLanguage(i18nBundleId: String? = nil, bundle: I18nBundle) {
    ...
    internal static func bundleFor(i18nBundleId: String? = nil) -> I18nBundle? {
        let bundleId = i18nBundleId ?? I18n.defaultI18nBundleId
        if bundleId.isEmpty {
            return nil
        }
        let lprojBundle = I18n.shared.lprojBundleMap.first(where: { $0.key == bundleId })?.value
        return lprojBundle
    }
}
